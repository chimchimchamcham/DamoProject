...
#eating_gram, #eating_Fat, #eating_Kcal, #eating_Carbo, #eating_Pro,
	#exe_time, #exe_Kcal, #eating_gram_myself {
	padding-top: 5px;
}
	...
	<div class="container-fluid">
		<div class="container" id="content_head">
			<div class="row">
				...
				<!-- 검색창 -->
				<div class="col-6" id="search_insert">
		...
		<div class="container" id="content_all">
			...
			<div id="content_exe">
				...
				<div class="row">
					<div class="col-3" style="width: 540px; height: 43px"></div>
				</div>
				<div class="row">
					<div class="col-3" style="width: 540px; height: 43px"></div>
				</div>
			...
			<div id="content_water">
				...
				<div class="row">
					<div class="col-3" style="width: 540px; height: 45px"></div>
				</div>
			</div>
			<!-- --------------------------------- -->
...
<script>
	...
	$('select[name=selectAdd]').change(function() {
						...
						if ($(this).val() == 'breakfast'
								|| $(this).val() == 'morning_snack'
								|| $(this).val() == 'lunch'
								|| $(this).val() == 'afternoon_snack'
								|| $(this).val() == 'dinner'
								|| $(this).val() == 'late_night_meal') {
							selectMenu = $(this).val();
							selectMenu = 'foodlist';
							$("#content_exe").hide();
							$("#content_water").hide();
							...
							$("#search_food").show();
							$("#search").hide();
							$("input[name=search_insert]").attr("disabled",
									false);
							//운동 선택시
							...
							selectMenu = 'water';
							$(
									"#content_eat,#content_exe,#search_inser,#search_btnt,#search")
									.hide();
									...
									success : function(data) {
										...
										if (selectMenu == 'foodlist') {
											for (var i = 0; i < data.resultList.length; i++) {
												...
												content += '<span class="foodName">'
														+ data.resultList[i].food_name
														+ '</span>';
												content += ' ('
														+ '<span class="food_weight">'
														+ data.resultList[i].food_weight
														+ '</span>' + 'g)';
												content += '<span class="food_calorie" style="display:none">'
														+ data.resultList[i].food_calorie
														+ '</span>';
												content += '<span class="food_carbo" style="display:none">'
														+ data.resultList[i].food_carbo
														+ '</span>';
												content += '<span class="food_protein" style="display:none">'
														+ data.resultList[i].food_protein
														+ '</span>';
												content += '<span class="food_fat" style="display:none">'
														+ data.resultList[i].food_fat
														+ '</span>';
												content += '</li>';
											}
											...
											for (var i = 0; i < data.resultList.length; i++) {
												content += '<li class="list-group-item" style="height: 30px; vertical-align: middle; padding:0">';
												content += '<span class="met_name">'
														+ data.resultList[i].met_name
														+ '</span>';
												content += '<span class="met" style="display:none">'
														+ data.resultList[i].met
														+ '</span>';
												content += '</li>';
											}
										}
	...
	$("#search_ul").on("click","li",function() {
				...
				if ($(this)[0].firstChild.className == "empty") {
					...
					var searchIns = $("input[name=search_insert]").val();
					
					if (selectMenu == 'foodlist') {
						console.log("음식 직접 입력");	
						
						$("input[name=search_food]").val(searchIns); 
						...
						$("input[name=search_food]").val($("input[name=search_insert]").val()); 
						exe_name=$("input[name=search_insert]").val();
						$("#content_exe").show();
						$("input[name=exe_time]").val('');
						$("#exeKcal_span").text('');
					}
					...
					if (selectMenu == 'foodlist') {
						$("#content_eat").show();
						$("#default_msg")
								.text($(this).find(".foodName").text());
						$("input[name=eating_gram]").val(
								$(this).find(".food_weight").text());
						$("#default_Kcal").text(
								$(this).find(".food_calorie").text());
						$("#default_Carbo").text(
								$(this).find(".food_carbo").text());
						$("#default_Pro").text(
								$(this).find(".food_protein").text());
						$("#default_Fat")
								.text($(this).find(".food_fat").text());
						...
						$("#default_msg")
								.text($(this).find(".met_name").text()); //선택한 운동으로 이름 바꾸기
					}
				}
	...
	$("input[name=eating_gram]").focusout(function() {
				...
				if (!emptyYN) {
					...
					if ($(this).val() != $(this).prop('defaultValue')) {
						...
						console.log(gram + "/" + weight + "/" + kcal + "/"
								+ changeKcal + "/" + carbo + "/" + protein
								+ "/" + fat);
	...
	$("input[name=search_food]").focusout(function(){
		if(selectMenu=='foodlist'){
			food_name=$(this).val();
		}else if(selectMenu=='met'){
			exe_name=$(this).val();
	...
	$("input[name=eating_Kcal]").focusout(function(){
		kcal=$(this).val();
		checkAll();
	});
	
	$("input[name=eating_Carbo]").focusout(function(){
		carbo=$(this).val();
		checkAll();
	});
	
	$("input[name=eating_Pro]").focusout(function(){
		protein=$(this).val();
		checkAll();
	});
	
	$("input[name=eating_Fat]").focusout(function(){
		fat=$(this).val();
		checkAll();
	});
	//-------------------------------------------------------------------------//
	...
	//운동 소모 칼로리 벗어남
	$("input[name=exe_Kcal]").focusout(function(){
		exeKcal=$(this).val();
		checkAll();
	});
	...
	$("#submitBtn").click(function() {
		...
		//분류 코드 구분 
		if (selectType == 'breakfast') {
			d_code = 'HD001'
		} else if (selectType == 'morning_snack') {
			d_code = 'HD002'
		} else if (selectType == 'lunch') {
			d_code = 'HD003'
		} else if (selectType == 'afternoon_snack') {
			d_code = 'HD004'
		} else if (selectType == 'dinner') {
			d_code = 'HD005'
		} else if (selectType == 'late_night_meal') {
			d_code = 'HD006'
	...
	function checkAll(){
		...
		if(selectMenu == 'foodlist'){
			if($("input[name=search_food]").val() == null || $("input[name=search_food]").val() =='' ){
				checker=false;
			}else if($("input[name=eating_gram]").val() ==null || $("input[name=eating_gram]").val() =='' ){
				checker=false;
			}else if($("input[name=eating_Kcal]").val() == null || $("input[name=eating_Kcal]").val() =='' ){
				checker=false;
			}else if($("input[name=eating_Carbo]").val() ==null || $("input[name=eating_Carbo]").val() =='' ){
				checker=false;
			}else if($("input[name=eating_Pro]").val() ==null || $("input[name=eating_Pro]").val() =='' ){
				checker=false;
			}else if($("input[name=eating_Fat]").val() ==null || $("input[name=eating_Fat]").val() =='' ){
				checker=false;
			}
		}else if(selectMenu == 'met'){
			if($("input[name=search_food]").val() == null || $("input[name=search_food]").val() =='' ){
				checker=false;
			}else if($("input[name=exe_Kcal]").val() == null || $("input[name=exe_Kcal]").val() ==''){
				checker=false;
			}else if($("input[name=exe_time]").val() == null || $("input[name=exe_Kcal]").val() ==''){
				checker=false;
			}
		}
