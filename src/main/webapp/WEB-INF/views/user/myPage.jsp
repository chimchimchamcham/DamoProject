	...
	<div class="container mt-4">
		<div class="row">
			<div id="userInfo" class="col-md-4 bg-white shadow">
				...
				<div id="profileTop">
					<c:if test="${dto.u_fileName eq 'default-profile.png' }">
						<img id="profileTopImg" class="rounded-circle"
							src="resources/img/${dto.u_fileName }">
					</c:if>
					<c:if test="${dto.u_fileName ne 'default-profile.png' }">
						<img id="profileTopImg" class="rounded-circle"
							src="/photo/${dto.u_fileName }">
					</c:if>
					<div id="profileTopNick">${dto.u_nick }</div>
					<c:if test="${sessionScope.loginId eq dto.u_id }">
						<button id="userInfoUpdateBtn" type="button"
							class="btn btn-secondary btn-sm">수정</button>
					</c:if>
					<img id="profileTopGradeImg"
						src="resources/img/${dto.g_fileName}.png" class="rounded-circle">
					<div id="profileTopRanking">
						<span>${dto.ranking }</span><span>위</span>
					</div>
				</div>
				...
				<div id="profileTable">
					<c:if test="${sessionScope.loginId eq dto.u_id }">
						<table>
							...
							<tr>
								<th>나이</th>
								<td><span>${dto.u_age }</span><span>세</span></td>
							</tr>
							<tr>
								<th>키</th>
								<td><span>${dto.u_height }</span><span>cm</span></td>
							</tr>
							<tr>
								<th>몸무게</th>
								<td><span>${dto.u_weight }</span><span>kg</span></td>
							</tr>
							<tr>
								<th>목표 몸무게</th>
								<td><span>${dto.u_tarWeight }</span><span>kg</span></td>
							</tr>
						</table>
					</c:if>
				</div>
				...
				<c:if test="${sessionScope.loginId eq dto.u_id }">
					<div class="mt-3">
						<a href="#" id="userBlind" class="link-primary mt-3">회원탈퇴</a>
					</div>
				</c:if>
				<!-- 신고하기(타인프로필) -->
				<c:if
			...
			<div id="fitSection" class="col-md-8">
				<!-- 통계 -->
				<div id="fitStatistics"
					class="row ml-0 mr-0 pt-2 pb-2 bg-white shadow">
					<div class="fitStatisticsCol col">
						<p class="text-secondary">질문 수</p>
						<p>
							<span>${dto.fitcnt }</span><span>개</span>
						</p>
					</div>
					<div class="fitStatisticsCol col">
						<p class="text-secondary">답변 수</p>
						<p>
							<span>${dto.anscnt }</span><span>개</span>
						</p>
					</div>
					<div class="fitStatisticsCol col">
						<p class="text-primary">채택 수</p>
						<p>
							<span>${dto.u_chooseCnt }</span><span>개</span>
						</p>
					</div>
					<div class="fitStatisticsCol fitStatisticsColLast col">
						<p class="text-danger">채택률</p>
						<p>
							<span>${dto.choosePct }</span><span>%</span>
						</p>
					</div>
				</div>
				...
				<div id="fitMenu" class="bg-white mt-3">
					...
					<ul class="nav nav-tabs" role="tablist">
						<li class="nav-item mr-2"><a
							class="nav-link nav-links navSelected rounded-top"
							data-toggle="tab" href="#myFitWrite">내가 쓴 Fit</a></li>
						<li class="nav-item mr-2"><a
							class="nav-link nav-links rounded-top" data-toggle="tab"
							href="#myFitAnswer">내가 쓴 답변</a></li>
						<c:if test="${sessionScope.loginId eq dto.u_id }">
							<li class="nav-item mr-2"><a
								class="nav-link nav-links rounded-top" data-toggle="tab"
								href="#myFitDictionary">내 사전</a></li>
						</c:if>
					</ul>
					...
					<div class="tab-content">
						<div id="myFitWrite"
							class="container tab-pane active overflow-auto shadow pl-4 pr-4">
							<br>
							...
							<c:forEach items="${myFitList }" var="myFit">
								<div
									class="container shadow items 
                            		<c:if test="${myFit.k_solutionYN eq 'Y'}"> solution</c:if>"
									onclick="location.href='fitDetail?k_no=${myFit.k_no}'">
									<div class="itemsTop container pt-3">
										<span
											class=" 
	                                    	<c:if test="${myFit.k_solutionYN eq 'Y'}"> text-success</c:if>
	                                    	<c:if test="${myFit.k_solutionYN ne 'Y'}"> text-primary</c:if>">Q.
										</span> <span class="title">${myFit.k_title }</span> <span
											class="category text-secondary float-right">${myFit.c_name }</span>
									</div>
									<div class="itemsBottom container pt-2">
						...
						<div id="myFitAnswer"
							class="container tab-pane fade overflow-auto shadow">
							<br>
							<c:forEach items="${myAnsList }" var="myAns">
								<div
									class="container shadow items 
                            		<c:if test="${myAns.kR_chooseYN eq 'Y'}"> solution</c:if>"
									onclick="location.href='fitDetail?k_no=${myAns.k_no}'">
									<div class="itemsTop container pt-3">
										<span class="text-secondary">Q. </span> <span
											class="title text-secondary">${myAns.k_title }</span> <span
											class="category text-secondary float-right">${myAns.c_name}</span>
									</div>
									<div class="itemsBottom container pt-2">
										<span
											class=" 
										...
										</span> <span>${myAns.kR_content }</span>
									</div>
								</div>
							</c:forEach>
						</div>
						<c:if test="${sessionScope.loginId eq dto.u_id }">
							<div id="myFitDictionary"
								class="container tab-pane fade overflow-auto shadow">
								<br>
								<c:forEach items="${myDirList }" var="myDir">
									<div
										class="container shadow items 
                            		<c:if test="${myDir.k_solutionYN eq 'Y'}"> solution</c:if>"
										onclick="location.href='fitDetail?k_no=${myDir.k_no}'">
										<div class="itemsTop container pt-3">
											<span
												class=" 
	                                    	<c:if test="${myDir.k_solutionYN eq 'Y'}"> text-success</c:if>
	                                    	<c:if test="${myDir.k_solutionYN ne 'Y'}"> text-primary</c:if>">Q.
											</span> <span class="title">${myDir.k_title }</span> <span
												class="category text-secondary float-right">${myDir.c_name }</span>
										</div>
										<div class="itemsBottom container pt-2">
